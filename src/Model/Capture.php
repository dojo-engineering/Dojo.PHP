...
class Capture implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    */
     ...
     * Sets capture_id
     *
     * @param string|null $capture_id The unique identifier for the capture.
     *
     * @return self
     */
    public function setCaptureId($capture_id)
    {
        if (is_null($capture_id)) {
            array_push($this->openAPINullablesSetToNull, 'capture_id');
        } else {
            $nullablesSetToNull = $this->getOpenAPINullablesSetToNull();
            $index = array_search('capture_id', $nullablesSetToNull);
            if ($index !== FALSE) {
                unset($nullablesSetToNull[$index]);
                $this->setOpenAPINullablesSetToNull($nullablesSetToNull);
            }
